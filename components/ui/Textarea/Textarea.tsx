		...
		return (
			<div>
				{label && (
					<label
						htmlFor={id}
						className={`${rest.disabled ? "opacity-15" : "opacity-100"} mb-3 block text-left font-IbmPlex text-base font-semibold leading-6 md:text-md`}
					>
						{label}
					</label>
				)}
					...
					<textarea
						...
						className={
							error
								...
								: `block w-full border border-border-input-dark bg-background px-p-input py-0 text-left font-IbmPlex text-sm placeholder-primary placeholder:text-left placeholder:opacity-40 focus:border-ring focus:outline-none focus:ring-1 focus:ring-ring-dark disabled:cursor-not-allowed disabled:opacity-50 md:text-md ${className}`
						}
						ref={ref}
						id={id}
						{...rest}
